...
const icons = [LayoutDashboard, Users, FileCheck, Fuel, ClipboardList, Smartphone, TrendingUp, Wrench];

const FleetManagement = () => {
  const { content: hero } = usePageContent("fleet_mgmt_hero");
  const { content: feat } = usePageContent("fleet_mgmt_features");
  const { content: cta } = usePageContent("fleet_mgmt_cta");
        ...
        <div className="container-premium relative">
          <motion.div className="max-w-3xl" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <div className="flex flex-wrap gap-4">
              <Button asChild size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground"><Link to="/get-quote">Get a Quote</Link></Button>
              <Button asChild variant="outline" size="lg"><Link to="/book-demo">Book a Demo</Link></Button>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding bg-accent/5">
        <div className="container-premium text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Button asChild size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground"><Link to="/get-quote">{String(cta.primaryCta ?? "Get a Quote")}</Link></Button>
            <Button asChild variant="outline" size="lg"><Link to="/book-demo">{String(cta.secondaryCta ?? "Book a Demo")}</Link></Button>
          </div>
        </div>
      </section>
    </PageWrapper>
