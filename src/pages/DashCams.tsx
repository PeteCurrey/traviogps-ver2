...
const icons = [Video, Wifi, Shield, HardDrive, Eye, AlertTriangle, Camera, Cloud];

const DashCams = () => {
  const { content: hero } = usePageContent("dashcams_hero");
  const { content: feat } = usePageContent("dashcams_features");
  const { content: cta } = usePageContent("dashcams_cta");
      ...
      <section className="section-padding bg-accent/5">
        <div className="container-premium text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Button asChild variant="outline" size="lg"><Link to="/pricing">{String(cta.secondaryCta ?? "View Pricing")}</Link></Button>
          </div>
        </div>
      </section>
    </PageWrapper>
